...
*Setup instructions for a gitian build of XVIC using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the XVIC
...
Clone the git repositories for xvic and gitian and then checkout the xvic version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the xvic repository
...
To build XVIC (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the xvic repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/xvic/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/XVictus-project/XVIC
...
For example:
```bash
URL=https://github.com/crowning-/xvic.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit xvic=${COMMIT} --url xvic=${URL} ../xvic/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit xvic=${COMMIT} --url xvic=${URL} ../xvic/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit xvic=${COMMIT} --url xvic=${URL} ../xvic/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/xvic-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/xvic-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/xvic-build.assert
```
...
[xvic/gitian.sigs](https://github.com/xvic-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
