...
#if defined(NDEBUG)
#error "XVIC cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uxvic) are considered zero fee (for relaying and mining)
...
{
    //Check to see if the zXVICis properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zXVICspend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zXVICwithout the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zXVICspend with serial %s is already in block %d\n", __func__,
    ...
    if (!spend.HasValidSerial(Params().Zerocoin_Params(fUseV1Params)))
        return error("%s : zXVICspend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zXVICspend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zxvic");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zXVICmints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    if (nHeight == 0) {
        ...
        return COIN * 1;
    } else if (nHeight <= 131400 && nHeight >= Params().LAST_POW_BLOCK()) {
        return COIN * 15;
    } else if (nHeight <= 219000 && nHeight > 131400) {
        return COIN * 14;
    } else if (nHeight <= 306600 && nHeight > 219000) { 
        return COIN * 13;
    } else if (nHeight <= 525600 && nHeight > 306600) {
        return COIN * 12;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, bool isZXVICStake)
{
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from XVIC
...
{
    RenameThread("xvic-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZXVICMinted()
{
...
void RecalculateZXVICSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zXVICsupply
...
bool RecalculateXVICSupply(int nHeightStart)
{
...
bool UpdateZXVICSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    //Track zXVICmoney supply in the block index
    if (!UpdateZXVICSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zXVICsupply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zXvicSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert XVICto zXVIC
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zXVICspends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zXVICserial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zXVICstake block is less than allowed start height", __func__);

    if (CZXvicStake* zXVIC= dynamic_cast<CZXvicStake*>(stake)) {
        CBlockIndex* pindexFrom = zXVIC->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zXVICstake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zXVICstake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zXVIC->GetValue()));
        if (nChecksum200 != zXVIC->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zXVIC->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZXVIC() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zXVICfails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zXVICmints and %d zXVICspends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // XVIC: We use certain sporks during IBD, so check to see if they are
