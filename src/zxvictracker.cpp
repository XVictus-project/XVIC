...
CzXVICTracker::CzXVICTracker(std::string strWalletFile)
{
...
CzXVICTracker::~CzXVICTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzXVICTracker::Init()
{
...
bool CzXVICTracker::Archive(CMintMeta& meta)
{
...
bool CzXVICTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzXVICTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzXVICTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzXVICTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzXVICTracker::GetSerialHashes()
{
...
CAmount CzXVICTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzXVICTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzXVICTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzXVICTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzXVICTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzXVICTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzXVICTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzXVICTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzXVICTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzXVICTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzXVICTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzXVICTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzXVICTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzXVICTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzXVICTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzXVICTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzXVICTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzxvic from DB\n", __func__, listDeterministicDB.size());
    }
