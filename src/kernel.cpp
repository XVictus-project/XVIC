...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZXvicStake(spend));
    } else {
        ...
        CXvicStake* xvicInput = new CXvicStake();
        xvicInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(xvicInput);
    }
