...
#ifndef XVIC_ZXVICWALLET_H
#define XVIC_ZXVICWALLET_H
...
public:
    CzXVICWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZXVIC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZXVIC(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //XVIC_ZXVICWALLET_H
