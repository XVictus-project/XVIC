...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered xvic address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 XVIC.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal xvic:URI (xvic:XyZ...?message=example)
            {
...
static void NotifyzXVICReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzXVICReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzXVICReset.connect(boost::bind(NotifyzXVICReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzXVICReset.disconnect(boost::bind(NotifyzXVICReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zxvicTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
