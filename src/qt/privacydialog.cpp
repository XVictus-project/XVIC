                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zXVIC ought to be enough for anybody." - Bill Gates, 2017
    ui->zXVICpayAmount->setValidator( new QDoubleValidator(0.0, 14190000.0, 20, this) );
    ...
    ui->labelzXVICSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zXVICpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzXVIC_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zXVIC is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zXVIC, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zXVIC...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zXVIC in ") +
...
void PrivacyDialog::on_pushButtonSpendzXVIC_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zXVIC is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zXVIC, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zXVIC
        sendzXVIC();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zXVIC
    sendzXVIC();
}

void PrivacyDialog::on_pushButtonZXvicControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZXvicControlDialog* zXvicControl = new ZXvicControlDialog(this);
    zXvicControl->setModel(walletModel);
    zXvicControl->exec();
}

void PrivacyDialog::setZXvicControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzXvicSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzXVIC()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid XVIC Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zXVICpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zXVIC is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " XVIC </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zXVIC</b>";
    ...
    // use mints from zXVIC selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZXvicControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZXvicControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zXVIC require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zXVIC"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZXVIC_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zXVIC require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zXVIC"));
            ui->TEMintStatus->repaint();
            return;
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zXVIC transaction
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zXVIC was spent successfully update the addressbook with the label
    ...
    // Clear zxvic selector in case it was used
    ZXvicControlDialog::setSelectedMints.clear();
    ui->labelzXvicSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zXVIC Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Xvic, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zXVIC Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zxvicTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zXVIC </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zXVIC "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zXVIC "));
    ui->labelzXVICAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zXVIC </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zXVIC </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzXVICSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzXVIC->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zXVIC
        ui->pushButtonMintzXVIC->setEnabled(false);
        ui->pushButtonMintzXVIC->setToolTip(tr("zXVIC is currently disabled due to maintenance."));

        // Spend zXVIC
        ui->pushButtonSpendzXVIC->setEnabled(false);
        ui->pushButtonSpendzXVIC->setToolTip(tr("zXVIC is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zXVIC
        ui->pushButtonMintzXVIC->setEnabled(true);
        ui->pushButtonMintzXVIC->setToolTip(tr("PrivacyDialog", "Enter an amount of XVIC to convert to zXVIC", 0));

        // Spend zXVIC
        ui->pushButtonSpendzXVIC->setEnabled(true);
        ui->pushButtonSpendzXVIC->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
