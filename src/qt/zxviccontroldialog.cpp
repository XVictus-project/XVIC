...
// Copyright (c) 2017-2018 The XVIC developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zxviccontroldialog.h"
#include "ui_zxviccontroldialog.h"
...
std::set<std::string> ZXvicControlDialog::setSelectedMints;
std::set<CMintMeta> ZXvicControlDialog::setMints;

ZXvicControlDialog::ZXvicControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZXvicControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZXvicControlDialog::~ZXvicControlDialog()
{
    delete ui;
}

void ZXvicControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZXvicControlDialog::updateList()
{
...
void ZXvicControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZXvicControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZXvic_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZXvicControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZXvicControlDialog::GetSelectedMints()
{
...
void ZXvicControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
