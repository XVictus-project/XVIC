...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::XVIC);
    ...
    if (!settings.contains("nAnonymizeXVICAmount"))
        settings.setValue("nAnonymizeXVICAmount", 1000);

    nAnonymizeXVICAmount = settings.value("nAnonymizeXVICAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeXVICAmount"))
        SoftSetArg("-anonymizexvicamount", settings.value("nAnonymizeXVICAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in xvic.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeXVICAmount:
            return QVariant(nAnonymizeXVICAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeXVICAmount:
            nAnonymizeXVICAmount = value.toInt();
            settings.setValue("nAnonymizeXVICAmount", nAnonymizeXVICAmount);
            emit anonymizeXVICAmountChanged(nAnonymizeXVICAmount);
            break;
        case CoinControlFeatures:
