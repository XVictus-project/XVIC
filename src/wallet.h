...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT1000IFMN = 3,
    ONLY_NONDENOMINATED_NOT1000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 XVICat the same time
    ONLY_1000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zXVICsend
enum ZerocoinSpendStatus {
    ZXVIC_SPEND_OKAY = 0,                            // No error
    ZXVIC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZXVIC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZXVIC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZXVIC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZXVIC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZXVIC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZXVIC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZXVIC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZXVIC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZXVIC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZXVIC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZXVIC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZXVIC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZXVIC_SPENT_USED_ZXVIC= 14,                      // Coin has already been spend
    ZXVIC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZXVIC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZXvicBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZXVICOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzxvicAuto) const;
    ...
    */
    ...
    CzXVICWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzXVICTracker> zxvicTracker;
    ...
    void setZWallet(CzXVICWallet* zwallet)
    {
        zwalletMain = zwallet;
        zxvicTracker = std::unique_ptr<CzXVICTracker>(new CzXVICTracker(strWalletFile));
    }

    CzXVICWallet* getZWallet() { return zwalletMain; }
    ...
    void setZXvicAutoBackups(bool fEnabled)
    {
    ...
    /** zXVICreset */
    boost::signals2::signal<void()> NotifyzXVICReset;
