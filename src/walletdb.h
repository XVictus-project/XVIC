...
public:
    ...
    bool WriteZXVICSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZXVICSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZXVICSeed_deprecated(uint256& seed);
    bool EraseZXVICSeed();
    bool EraseZXVICSeed_deprecated();

    bool WriteZXVICCount(const uint32_t& nCount);
    bool ReadZXVICCount(uint32_t& nCount);
