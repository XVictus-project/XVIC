...
CzXVICWallet::CzXVICWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zxvic seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZXVICSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZXVICSeed_deprecated()) {
                    LogPrintf("%s: Updated zXVICseed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zxvic seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zxvic wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzXVICWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZXVICCount(nCountLastUsed);
    else if (!walletdb.ReadZXVICCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzXVICWallet::Lock()
{
    seedMaster = 0;
}

void CzXVICWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzXVICWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZXVIC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzXVICWallet::LoadMintPoolFromDB()
{
...
void CzXVICWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzXVICWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzXVICWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zxvicTracker->HasPubcoinHash(pMint.first)) {
...
bool CzXVICWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZXVIC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zxvicTracker which also adds to database
    pwalletMain->zxvicTracker->Add(dMint, true);
...
void CzXVICWallet::SeedToZXVIC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzXVICWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzXVICWallet::GenerateDeterministicZXVIC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzXVICWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzXVICWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
